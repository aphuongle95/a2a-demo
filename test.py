...
from agent import attractions_agent, flight_agent, hotel_agent
...
async def run_cli_flights():
    ...
    session_service = InMemorySessionService()
    credential_service = InMemoryCredentialService()
    # 1. Clone the agent first (without passing the problematic 'tools' argument).
    toolless_flight_agent = flight_agent.clone() 
    
    # 2. Directly overwrite the .tools attribute on the cloned object to an empty list.
    # This removes the problematic function, bypassing the parser's ValueError.
    toolless_flight_agent.tools = [] 
    
    # 3. Use the newly modified agent in the App wrapper.
    app = App(name="TravelPlanner", root_agent=toolless_flight_agent)
...
async def run_cli_hotels():
    ...
    # --- THE FIX STARTS HERE ---
    # 1. Clone the agent first (without passing the problematic 'tools' argument).
    toolless_flight_agent = hotel_agent.clone() 
    ...
    # since the specific flight tool is now disabled.
    user_input = "Suggest hotels in Paris with at least 4 stars and under $120" 
    
    # --- The rest of the setup and execution logic remains the same ---
